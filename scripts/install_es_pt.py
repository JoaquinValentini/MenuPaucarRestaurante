# scripts/install_es_pt.py (versión con trazas detalladas en UTF-8)
import sys

def log(msg):
    print(msg, flush=True)  # salida inmediata

print("== INICIO install_es_pt.py ==")
...
try:
    log(" Buscando paquete espt en el índice...")
    available = pkg.get_available_packages()
    match = None
    for p in available:
        # Sirve 'pt' o 'pt_br' (cualquier to_code que empiece con 'pt')
        if str(p.from_code).startswith("es") and str(p.to_code).startswith("pt"):
            match = p
            break

    if not match:
        print(" No encontré paquete espt en el índice.")
...
try:
    ...
    pt = next((l for l in installed if str(l.code).startswith("pt")), None)
    if es and pt:
        t = es.get_translation(pt)
        result = t.translate("Hola, ¿cómo estás?")
        log(f"Prueba de traducción ESPT: {result}")
    else:
        log(" No se detectaron 'es' y/o 'pt' en los idiomas instalados.")
except Exception as e:
    print(" ERROR en la prueba de traducción:", e)
    sys.exit(5)

print("== FIN install_es_pt.py ==")
